function [Voltage_output, phasors] = ieee34_iter(loads, Zbus, Cbus)

tapA814R = 1.0 + 0.00625 * 0;
tapB814R = 1.0 + 0.00625 * 0;
tapC814R = 1.0 + 0.00625 * 0;
alpha814R = [tapA814R; tapB814R; tapC814R];
alphaM814R = alpha814R * alpha814R';

tapA852R = 1.0 + 0.00625 * 0;
tapB852R = 1.0 + 0.00625 * 0;
tapC852R = 1.0 + 0.00625 * 0;
...
v_lb <= diag(A * v888 * ctranspose(A)) <= v_ub;
...
v_lb <= diag(v838) <= v_ub;
v_lb <= diag(A * v890 * ctranspose(A)) <= v_ub;
...
v888 == v832 - S832888*ctranspose(Z832888) - Z832888*ctranspose(S832888) + Z832888*l832888*ctranspose(Z832888);
...
A * v814R * AH == (A * v814([1, 2, 3],[1, 2, 3]) * AH) .* alphaM814R;
...
A * v852R * AH == (A * v852([1, 2, 3],[1, 2, 3]) * AH) .* alphaM852R;
...
V888 = V832([1, 2, 3]) - A * Z832888* AH *I832888;
...
phasors=[phasors;recover_voltage(V888, 123)];
phasors=[phasors;recover_voltage(V864, 1)];
phasors=[phasors;recover_voltage(V834, 123)];
phasors=[phasors;recover_voltage(V890, 123)];
